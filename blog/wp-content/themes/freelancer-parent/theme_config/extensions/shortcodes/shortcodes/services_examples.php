...
function tfuse_services_examples($atts, $content = null) {
    $out = '';
    extract(shortcode_atts(array('title' => ''), $atts));
    $get_services = do_shortcode($content);
    $uniq = rand(1,99);
    if($title != '') $out .= '<h3 class="services-title">'.$title.'</h3>';
    $out .= '<div class="examples">
        <ul class="work-examples work-examples'.$uniq.'">'.$get_services.'</ul>
        <div class="clearfix"></div>
        <a class="up work-examples-next'.$uniq.'" href="#"><span>'.__('MORE +','tfuse').'</span></a><a class="down work-examples-prev'.$uniq.'" href="#"><span>'.__('PREV -','tfuse').'</span></a>
    </div>
    <script>
        jQuery(window).load(function () {
            function carousel_call() {
                jQuery(".work-examples'.$uniq.'").carouFredSel({
                    auto: false,
                    items: 7,
                    debug: true,
                    prev: ".work-examples-prev'.$uniq.'",
                    next: ".work-examples-next'.$uniq.'",
                    direction: "up",
                    scroll: {
                        items: 1,
                        easing: "linear",
            ...
            carousel_call();
            jQuery(".services-carousel").on("slide.bs.carousel", function () {
                setTimeout(function(){carousel_call();},300);
            })
        });
    </script>';
...
$atts = array(
    'name' => __('Services examples','tfuse'),
    'desc' => __('Here comes some lorem ipsum description for the shortcode.','tfuse'),
    'category' => 8,
    'options' => array(
        array(
            'name' => __('Title','tfuse'),
            'desc' => __('Title for services examples','tfuse'),
            'id' => 'tf_shc_services_examples_title',
            'value' => '',
            'divider' => TRUE,
            'type' => 'text'
        ),
        array(
            'name' => __('Name','tfuse'),
            'desc' => '',
            'id' => 'tf_shc_services_examples_name',
            'value' => '',
        ...
        array(
            'name' => __('URL','tfuse'),
            'desc' => '',
            'id' => 'tf_shc_services_examples_url',
            'value' => '',
...
tf_add_shortcode('services_examples', 'tfuse_services_examples', $atts);

function tfuse_se_tab($atts, $content = null) {
    extract(shortcode_atts(array('name' => '', 'url' => ''), $atts));
    $name = $content;
...
add_shortcode('se_tab', 'tfuse_se_tab', $atts);