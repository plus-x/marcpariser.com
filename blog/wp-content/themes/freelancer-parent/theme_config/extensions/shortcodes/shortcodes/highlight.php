...
function tfuse_highlight($atts, $content) {
    extract(shortcode_atts(array('bg' => '', 'color' => ''), $atts));
    $style = '';
    if (!empty($bg) || !empty($color))
        $style = 'style="background:'.$bg.'; color:'.$color.'" ';

    return '<span '.$style.'>' . strip_tags($content) . '</span>';
}
...
$atts = array(
    ...
    'options' => array(
        array(
            ...
            'desc' => __('Specify the background-color','tfuse'),
            'id' => 'tf_shc_highlight_bg',
            'value' => '#f5da2e',
            'type' => 'colorpicker'
        ),
        array(
            'name' => __('Text Color','tfuse'),
            'desc' => __('Specify the text color','tfuse'),
            'id' => 'tf_shc_highlight_color',
            'value' => '#000000',
            'type' => 'colorpicker'
        ),
