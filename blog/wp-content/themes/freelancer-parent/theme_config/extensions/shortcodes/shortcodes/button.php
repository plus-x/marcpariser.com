...
function tfuse_button($atts, $content = null)
{
    ...
    if ( !empty($bgcolor) || !empty($textcolor) || !empty($bordercolor))
    {
        $border = '';
        $class = 'button';
        if(!empty($bordercolor)) $border = 'border: 1px solid '.$bordercolor;
        $style = ' style="background-color:'.$bgcolor.';color:'.$textcolor.';'.$border.';"';
    }
    else
        $class = 'button ' . $class;

    if ( !empty($bgcolor) ) $span_style = ' style="color:'.$textcolor.';"';
    else $span_style = '';
...
$atts = array(
    ...
    'options' => array(
        array(
            'name' => __('Class','tfuse'),
            'desc' => __('Specifies one or more class names for an shortcode, separated by space.<br /><b>predefined classes:</b> pink,black,blue,yellow,green.','tfuse'),
