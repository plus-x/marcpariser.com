 ...
 * @return array
 */
function zircone_body_classes( $classes ) {
...
add_filter( 'body_class', 'zircone_body_classes' );
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	 ...
	 */
	function zircone_wp_title( $title, $sep ) {
		if ( is_feed() ) {
			return $title;
		}
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( __( 'Page %s', 'zircone' ), max( $paged, $page ) );
		}

		return $title;
	}
	add_filter( 'wp_title', 'zircone_wp_title', 10, 2 );
	 ...
	 */
	function zircone_render_title() {
		?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
		<?php
	}
	add_action( 'wp_head', 'zircone_render_title' );
endif;
