...
function tf_reservationform_shortcode($atts){
    ...
    if($form_exists){
        $out .='<div class="contact_form clearfix reservationForm">';
        ...
        foreach($form['input'] as $form_input){
            ...
            elseif($form['position']=='half_column')
                $ewidth=485-$lines[$element_line]+$margin;
            else
            ...
            if($lines[$element_line]==$dimension*2 && $form_input['width']>=40 && $form_input['width']<=90){
                $form_input['ewidthpx'] = (int)(30+($ewidth-$dimension)*$form_input['width']/100);
            }
            elseif($lines[$element_line]==$dimension*3 && $form_input['width']<40 && $form_input['width']>32){
                $form_input['ewidthpx'] = (int)(27+($ewidth-$dimension)*$form_input['width']/100);
            }
            elseif($lines[$element_line]==$dimension*4 && $form_input['width']<32 && $form_input['width']>23){
                $form_input['ewidthpx'] = (int)(25+($ewidth-$dimension)*$form_input['width']/100);
            }
            //

            // for one field in other line
            if($lines[$element_line]==$dimension && $form_input['width']>=40 && $form_input['width']<=90){
                $form_input['ewidthpx'] = (int)(10+($ewidth-$dimension)*$form_input['width']/100);
            }
            elseif($lines[$element_line]==$dimension && $form_input['width']<40 && $form_input['width']>32){
                $form_input['ewidthpx'] = (int)(14+($ewidth-2*$dimension)*$form_input['width']/100);
            }
            elseif($lines[$element_line]==$dimension && $form_input['width']<33){
                $form_input['ewidthpx'] = (int)(15+($ewidth-3*$dimension)*$form_input['width']/100);
            }
        ...
        $out.='<div class="clear"></div><div class="field_submit" style="width:100%">';
        $out.='<div class="contact_submit orange"><input type="submit" id="send_reservation" class="btn-submit button orange" name="submit" title="Submit mesage" value="'. __(urldecode($form['submit_mess']),'tfuse').'" /></div>';
        $out.='<a id="sending" href="#" class="button large orange" style="display: none;"><span>'.__('Sending ...','tfuse').'</span></a>
        <a class="reset_form" href="#" onclick="resetFields(this,event)">'.__(urldecode($form['reset_button']),'tfuse').'</a>
        <img id="sending_img" src="'.$surse.'" alt="Sending" style="margin-bottom:2px; margin-left:8px; display: none; border:0;" /></div>';
...
function res_text($input,$form_input){
    return "<div class='pull-left field_text ".$form_input['classes']."' style='".$form_input['floating']."'>
...
function res_textarea($input,$form_input){
    return "<div class='pull-left field_textarea ".$form_input['classes']."' style='".$form_input['floating']."'>
                <label class='label_title' for='" .TF_THEME_PREFIX.'_'.trim($form_input['shortcode']). "'>".__($form_input['label_text'],'tfuse')."</label>
                <textarea  style='width:".($form_input['ewidthpx'])."px;' class='textareaField ".$input['properties']['class']."' name='".TF_THEME_PREFIX.'_'.trim($form_input['shortcode'])."' rows='10' ></textarea>
            </div>";
}

function res_radio($input,$form_input){
    $checked='checked="checked"';
    $output="<div class='input_styled radiobox pull-left ".$form_input['classes']."' style='width:".($form_input['ewidthpx'])."px;".$form_input['floating']."'>
                <label class='label_title' for='" .TF_THEME_PREFIX.'_'.trim($form_input['shortcode']). "'>".__($form_input['label_text'],'tfuse')."</label>";
...
function res_datepicker($input,$form_input){
    ...
    $output="<div class='field_text pull-left ".$form_input['classes']."' style='".$form_input['floating']."'>
                 <label class='label_title' for='" .TF_THEME_PREFIX.'_'.trim($form_input['shortcode']). "'>".__($form_input['label_text'],'tfuse')."</label>
                 <input style='width:".($form_input['ewidthpx'])."px;' type='text' class='inputField ".$input['properties']['class']."' name='".TF_THEME_PREFIX.'_'. trim($form_input['shortcode'])."'/></div>";
    return $output;
}
function res_checkbox($input,$form_input){
    $checked = ($input['value'] == 'true') ? 'checked="checked"' : '';
    $output = "<div class='checkbox input_styled pull-left ".$form_input['classes']."' style='width:".($form_input['ewidthpx'])."px;".$form_input['floating']."'>
                <input class='".$input['properties']['class']."' style='width:13px;' type='checkbox' name='" .TF_THEME_PREFIX."_".trim($form_input['shortcode']). "' id='" .TF_THEME_PREFIX."_".trim($form_input['shortcode']). "' value='".$form_input['label']."'" . $checked . "/>
...
function res_captcha($input,$form_input){
    ...
    $out="<div class='captcha field_text pull-left' style='width:".$form_input['width']."%;".$form_input['floating']."'>
            <label class='label_title' for='" .TF_THEME_PREFIX.'_'.trim($form_input['shortcode']). "'>".__($form_input['label_text'],'tfuse')."</label>
...
function res_select($input,$form_input){
    ...
    $out = "<div class='field_select pull-left ".$form_input['classes']."' style='".$form_input['floating']."' >
                <label class='label_title' for='" .TF_THEME_PREFIX."_".trim($form_input['shortcode']). "'>".__($form_input['label_text'],'tfuse')."</label>
