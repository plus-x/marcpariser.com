 ...
 * line: true, false
 * 
 * [map lat="41.887" long="-87.630" zoom="10" type="map1" line="true"]
 * 
 * [map lat="41.887" long="-87.630" zoom="10" type="map2" line="true"]
 * 
 * [map height="500" lat="41.887" long="-87.630" zoom="3" type="map3" address="Chicago" line="true"]
 * 
 */
...
class TFUSE_Map_Shortcode {
    ...
    static function init() {
        ...
        $atts = array(
            ...
            'options' => array(
                ...
                array(
                    'name' => __('Show oblique line ?','tfuse'),
                    'desc' => __('Show oblique line ?','tfuse'),
                    'id' => 'tf_shc_map_line',
                    'value' => false,
                    'type' => 'checkbox'
                ),
    ...
    static function print_script() {
        ...
        wp_enqueue_scripts('maps.google.com');
        wp_enqueue_scripts('jquery.gmap');
    }

    static function handle_shortcode($atts) {
        self::$add_script = true;

        extract(shortcode_atts(array('width' => 500, 'height' => 350, 'lat' => 0,'long' => 0, 'zoom' => 12, 'type' => '', 'address' => '', 'line' => ''), $atts));
        $return = '';
        $rand = rand(600, 700);
        $width = (int) $width;
        $height = (int) $height;
        ...
        if($line == 'true')
            $return .= '<div class="contact-section"><div class="contact_map not-ie">
                <svg width="100%" height="230px">
                    <defs>
                        <mask id="mask" maskUnits="userSpaceOnUse" maskContentUnits="userSpaceOnUse">
                            <image width="350px" height="230px" xlink:href="'.get_template_directory_uri().'/images/polygon.png"></image>
                        </mask>
                    </defs>
                    <foreignObject width="100%" height="230px" style="mask:url(#mask);max-width: 100%">
                        <div id="map' . $rand . '" class="map frame_box" style="width: ' . $width . 'px; height: ' . $height . 'px; border: 1px solid #ccc; overflow: hidden;"></div>
                    </foreignObject>
                </svg><img src="'.get_template_directory_uri().'/images/aslanta-line2.png" alt="" class="map-aslant-line"/>
            </div>
            <div class="contact_map ie">
			    <div id="map' . ($rand+1) . '" class="map frame_box" style="width: ' . $width . 'px; height: ' . $height . 'px; border:none;box-shadow:none; overflow: hidden;"></div>
            </div>
            </div>
            <script>
                jQuery(window).load(function(){
                    jQuery("#map' . ($rand+1) . '").gMap({
                        markers: [{
                            latitude: ' . $lat . ',
                            longitude: ' . $long . '}],
                        zoom: ' . $zoom . ',
                ...
                function getInternetExplorerVersion()
                {
                  ...
                  var rv = -1;
                  if (navigator.appName == "Microsoft Internet Explorer" || isAtLeastIE11)
                  {
                    rv = 1;
                ...
                function checkVersion()
                {
                  ...
                  if ( ver > -1 )
                  {
                    jQuery(".contact_map.not-ie").hide();
                  }
                  else
                  {
                    jQuery(".contact_map.ie").hide();
                  }
                }
