...
function tfuse_contat_button($atts, $content = null)
{
    extract( shortcode_atts(array('type' => 'phone', 'bgcolor' => '', 'width' => '', 'textcolor' => ''), $atts) );
    $style = '';
    if ( !empty($bgcolor) || !empty($textcolor) || !empty($width))
    {
        $style = ' style="background-color:'.$bgcolor.';color:'.$textcolor.';width:'.$width.';"';
    }

    return '<div class="cb_wrap"><span '.$style.' class="contact_button '.$type.'"><img src="'.get_template_directory_uri().'/images/icons/'.$type.'-ico.png" alt=""/>'.$content.'</span></div>';
}

$atts = array(
    'name' => __('Contact Buttons','tfuse'),
    'desc' => __('Here comes some lorem ipsum description for the button shortcode.','tfuse'),
    'category' => 2,
    'options' => array(
        array(
            'name' => __('Type','tfuse'),
            'desc' => __('Select type of button','tfuse'),
            'id' => 'tf_shc_contact_button_type',
            'value' => 'phone',
            'options' => array(
                'phone' => __('Phone','tfuse'),
                'fax' => __('Fax','tfuse'),
                'email' => __('Email','tfuse'),
            ),
            'type' => 'select'
        ),
        array(
            ...
            'id' => 'tf_shc_contact_button_content',
            'value' => 'Contact Button',
            'type' => 'textarea'
        ),
        array(
            'name' => __('Width','tfuse'),
            'desc' => __('Specifies the width for the button. Ex: 300px','tfuse'),
            'id' => 'tf_shc_contact_button_width',
            'value' => '300px',
            'type' => 'text'
        ),
        array(
            ...
            'id' => 'tf_shc_contact_button_bgcolor',
            'value' => '#a65e00',
            'type' => 'colorpicker'
        ),
        array(
            ...
            'id' => 'tf_shc_contact_button_textcolor',
            'value' => '#fff',
            'type' => 'colorpicker'
        ),
    )
...
tf_add_shortcode('contact_button', 'tfuse_contat_button', $atts);