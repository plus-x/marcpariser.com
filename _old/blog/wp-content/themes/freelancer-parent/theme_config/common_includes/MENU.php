...
function tfuse_custom_menu($menu_type){
    // custom menu for show #Title in menu
    global $menus;
    if (isset($menus[$menu_type])) {
        if (($locations = get_nav_menu_locations()) && isset($locations['default'])) {
            $menu = wp_get_nav_menu_object($locations['default']);
            $menu_items = wp_get_nav_menu_items($menu->term_id);

            $out = '<nav class="topmenu clearfix"><ul>';
            foreach($menu_items as $item){
                $title = $item->title;
                $title_href = preg_replace('/[^A-Za-z0-9\ -]/', '_', $item->title);
                if($item->object == 'custom') {
                    $href = $item->url;
                }
                else{
                    $href = '#' . str_replace(' ', '_', $title_href);
                }
                $class = '';
                foreach($item->classes as $class_item){
                    $class .= ' '.$class_item;
                }
                $out .= '<li class="'.$href.' '.$class.'"><a href="'.$href.'">'.$title.'</a></li>';
            }
            $out .= '</ul></nav>';
            echo $out;
        }
...
function tfuse_custom_footer_menu($menu_type){
    // custom menu for show #Title in menu
    global $menus;
    if (isset($menus[$menu_type])) {
        if (($locations = get_nav_menu_locations()) && isset($locations['default'])) {
            ...
            $menu_items = wp_get_nav_menu_items($menu->term_id);
            $count = 0;
            $out = '<div class="footer_nav clearfix"><ul>';
            foreach($menu_items as $item){
                $count++;
                $title = $item->title;
                ...
                $out .= '<li class="'.$href.' '.$class.'"><a href="#'.$href.'" data-toggle="tooltip" title="'.$title.'"></a></li>';
            }
            $out .= '</ul></div>';
            $ul_width = ($count-1)*80 + $count*10;
            $out .= '<style>.footer_nav ul {width: '.$ul_width.'px;}</style>';
            echo $out;
        }
    }
}
