...
if (!function_exists('tfuse_media')) :
     ...
     */
    function tfuse_media($id=0,$return=false)
    {
        ...
        $tfuse_media['video_link']             = tfuse_page_options('video_link','',$id);
        $tfuse_media['enable_video']           = tfuse_page_options('enable_video',tfuse_options('enable_video'),$id);
        $tfuse_media['enable_image']           = tfuse_page_options('enable_image',tfuse_options('enable_image'),$id);

        if ( $tfuse_media['enable_image'] )
        {
            $tfuse_media['image']              = tfuse_page_options('single_image','',$id);
            $tfuse_media['img_dimensions']     = tfuse_page_options('single_img_dimensions',tfuse_options('single_img_dimensions'),$id);
            $tfuse_media['img_position']       = tfuse_page_options('single_img_position',tfuse_options('single_img_position'),$id);
        }

        $post_type = get_post_type($id);
        if ( !empty($tfuse_media['video_link'] ) && $tfuse_media['enable_video'] )
        {
            if($post_type=='work' && tfuse_page_options('work_type','image',$id)=='image'){
                $output = '';
            }
            elseif($post_type=='service' && tfuse_page_options('service_type','image',$id)=='image'){
                $output = '';
            }
            else{
                $tfuse_media['video_dimensions']   = tfuse_page_options('video_dimensions',tfuse_options('video_dimensions'),$id);
                $tfuse_media['video_position']     = tfuse_page_options('video_position',tfuse_options('video_position'),$id);
                ...
                $output .= $video->width($tfuse_media['video_dimensions'][0])->height($tfuse_media['video_dimensions'][1])->source($tfuse_media['video_link'])->get();
                $output .= '</div>';
            }
        }
        ...
        if($post_type=='work'){
            if($tfuse_media['image'] != '' && tfuse_page_options('work_type','image',$id)=='image'){
                $output = '<div class="slide-img  clearfix '.$tfuse_media['img_position'].'">
                    <img width="'.$tfuse_media['img_dimensions'][0].'" height="'.$tfuse_media['img_dimensions'][1].'" src="'.$tfuse_media['image'].'" alt="" data-animate="'.tfuse_page_options('animate_type','bounceInDown',$id).'" class="animate3"/>
                </div>';
            }
        }
        elseif($post_type=='service'){
            if($tfuse_media['image'] != '' && tfuse_page_options('service_type','image',$id)=='image'){
                $output = '<img width="'.$tfuse_media['img_dimensions'][0].'" height="'.$tfuse_media['img_dimensions'][1].'" src="'.$tfuse_media['image'].'" alt="" />';
            }
        }
        elseif($tfuse_media['enable_single_lightbox'])
            $output .= '<div class="post-image '.$tfuse_media['img_position'].'"><a data-rel="prettyPhoto" rel="prettyPhoto" href="'.$tfuse_media['image'].'">'.$tfuse_image.'</a></div>';
        else
            $output .= '<div class="post-image '.$tfuse_media['img_position'].'"><a href="'.$tfuse_media['image'].'">'.$tfuse_image.'</a></div>';
